...
export async function removeLiquidity(
  ...
  const { position: matchingLbPosition, info: matchingPositionInfo } =
    await meteora.getRawPosition(positionAddress, wallet.publicKey);

  if (!matchingLbPosition || !matchingPositionInfo) {
    throw fastify.httpErrors.notFound(`Position not found: ${positionAddress}`);
  }

  const dlmmPool = await meteora.getDlmmPool(
    matchingPositionInfo.publicKey.toBase58(),
  );
  if (!dlmmPool) {
  ...
  const binIdsToRemove = matchingLbPosition.positionData.positionBinData.map(
    (bin) => bin.binId,
  );
  ...
  const { balanceChange: tokenXRemovedAmount, fee } =
...
export const removeLiquidityRoute: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post<{
    ...
    async (request) => {
      try {
        const { network, address, positionAddress, percentageToRemove } =
          request.body;
